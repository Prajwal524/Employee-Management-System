...
<head>
    ...
    <title>View Departments — EMS</title>
...
<nav class="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm">
    <div class="max-w-4xl mx-auto px-6 flex items-center h-16 gap-4">
...
<!-- MAIN -->
<div class="max-w-4xl mx-auto px-6 py-10 page-content">
    <div class="mb-7">
        <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Departments</h1>
        <p class="text-slate-500 text-sm mt-1">All departments registered in the system</p>
    </div>

    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <%DeptDAO ddao=new DeptDAOImpl(); List<Dept> dList=ddao.findAll();%>

        <div class="px-6 py-4 border-b border-slate-100 flex items-center">
            <p class="text-sm font-bold text-slate-600">
                <span class="bg-violet-50 text-violet-700 border border-violet-100 text-xs font-extrabold px-2.5 py-1 rounded-full mr-2"><%=dList != null ? dList.size() : 0%></span>
                departments found
            </p>
        </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-slate-50">
                        <th class="text-left px-6 py-3.5 text-xs font-bold text-slate-500 uppercase tracking-wider">Dept No</th>
                        <th class="text-left px-6 py-3.5 text-xs font-bold text-slate-500 uppercase tracking-wider">Department Name</th>
                        <th class="text-left px-6 py-3.5 text-xs font-bold text-slate-500 uppercase tracking-wider">Location</th>
                    </tr>
                </thead>
                ...
                <tr>
                    <td class="px-6 py-4">
                        <span class="text-xs font-extrabold text-violet-600 bg-violet-50 border border-violet-100 px-2.5 py-1 rounded-full"><%=d.getDno()%></span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            
                            <span class="font-semibold text-slate-800 text-sm"><%=d.getDname()%></span>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-slate-600 text-sm"><%=d.getLocation()%></td>
                </tr>
                <%}
                }%>
                </tbody>
